//Removing Allocation of highest allocated process
//Main Code
#include<stdio.h>
...
void Bankeralgo()
{
  ...
  if(flag==0)
  {
  	...
  	RemoveLowestAllocProcess();				//deallocating instances from lowest Allocation process.
  	print();
  	Bankeralgo();
  }
...
//function to remove allocation of lowest Allocation process after getting deadlock.
int RemoveLowestAllocProcess () {
	int min = 0, minAllocation=0;
	for(int i=0;i<resource;i++)
	{
		minAllocation+=allocation[0][i];
	} 
	
	for(int i = 0; i < n; i++) {
		if( allocation[i][0]+allocation[i][1]+allocation[i][2] > minAllocation) {
			minAllocation = allocation[i][0]+allocation[i][1]+allocation[i][2] ;
			min = i;
		}
	}
